...
def main(ssh_enabled=False, polling_interval=10):
    ...
    observer = Observer()
    observer.schedule(event_handler_instance, str(config.BASE_PATH), recursive=False)
    try:
        observer.start()
    except Exception as e:
    ...
    with mujoco_viewer.launch_passive(
        ...
        while viewer.is_running():
            ...
            if viewer_state.change_rollout:
                # Clamp evaluation and environment indices.
                vu.reset_figures(metrics_keys)
            ...
            with viewer.lock():
                ...
                # Update UI only every 10 frames to prevent freezing
                if loop_count % 10 == 0:
                    # Overlay text.
                    text_1 = "Eval\nEnv\nStep\nStatus\nSpeed"
                    text_2 = (
                    ...
                    if viewer_state.show_metrics:
                        ...
                        viewport_figures = list(
                            zip(viewports, list(vu.figures.values()))
                        )
                ...
                if isinstance(cur_rollout.obs, Mapping):
                    if any(key.startswith("pixels/") for key in cur_rollout.obs.keys()):
                        if viewer_state.show_pixel_obs:
                            cur_obs = rollout.dict_obs_t_select(
                                cur_rollout.obs, replay_index
                            )
